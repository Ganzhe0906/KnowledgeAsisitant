...
export async function processKnowledge(
  ...
  onProgress?: (data: Record<string, unknown>) => Promise<void> | void
  ...
  if (!BIBIGPT_KEY || !GEMINI_KEY) {
    ...
    throw new Error(`未配置 API Key: ${missing.join(", ")}。`);
  }
...
【必须严格遵守的规则】
1. 标题结构必须严格遵循格式：【xxxx】xxxxx
2. 【】内的内容必须严格为4个中文字符，用于精准概括这个知识可以拿来干嘛的“应用场景”或“分类属性”（例如：【财经知识】、【时政关注】、【视频创意】、【效率提升】、【生活技巧】、【思维提升】、【商业实战】等）。
3. 后半部分（xxxxx）必须是一个完整的短语或句子，能够清晰描述事件、核心知识点或主题。标题总长度控制在 10 到 25 个字之间。
4. 直接输出标题，不要任何多余的字、额外标点符号、引号或解释。
5. 不要包含除【】以外的特殊文件系统不支持的符号（如 / \\ : * ? " < > |）。

【好标题示例】
- 【科技前沿】苹果发布M4芯片AI性能大幅提升
- 【健康养生】普通人如何通过三个习惯改善睡眠
- 【商业实战】2024年独立开发者的变现指南
- 【思维提升】如何用第一性原理解决复杂问题
